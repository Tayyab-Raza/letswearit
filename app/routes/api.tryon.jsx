...
export async function action({ request }) {
  ...
  try {
    ...
    // Only counts against the plan on a confirmed success.
    await recordGeneration(shop, { productId: gid, angle });

    return json({ angle, imageUrl: `data:image/jpeg;base64,${imageBase64}` });
  } catch (error) {
    return json(
      {
